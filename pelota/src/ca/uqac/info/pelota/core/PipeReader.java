...
import ca.uqac.info.pelota.core.EventNotifierPelota.MaximumEventsReachedException;
import ca.uqac.info.pelota.core.EventNotifierPelota.VerdictObtainedException;
import ca.uqac.info.pelota.core.PipeCallback;
import ca.uqac.info.pelota.core.PipeCallback.CallbackException;
import ca.uqac.info.pelota.xml.TokenBuffer;
import ca.uqac.info.pelota.xml.XmlTokenBuffer;
...
public class PipeReader implements Runnable
{
   ...
   */
  protected volatile EventNotifierPelota m_callback;
  ...
  public static final int ERR_MONITOR_STOP = 10; // The monitor stopped by itself
  
  /**
   * The verdict of the underlying monitor
   */
  protected String m_verdict = "?";
  ...
  public PipeReader(String pipeName, EventNotifierPelota t, boolean isFile)
  {
    this();
    m_callback = t;
    m_isFile = isFile;
  ...
  public PipeReader(String pipeName, EventNotifierPelota t)
  {
    this(pipeName, t, true);
  }
  
  public PipeReader(InputStream is, EventNotifierPelota t, boolean isFile)
  {
    this();
    m_callback = t;
    m_isFile = isFile;
  ...
  public void run()
  {
    ...
    try
    {
    	while (true)//while (m_callback.isRunning())
      {
        try
        {
    ...
    catch (VerdictObtainedException e)
    {
    	m_returnCode = ERR_MONITOR_STOP;
    	m_verdict = e.getVerdict();
    }
    catch (MaximumEventsReachedException e)
    {
    	m_returnCode = ERR_MONITOR_STOP;
    }
